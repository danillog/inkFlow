...
interface AppPoint { x: number; y: number; pressure: number; }
...
const DrawingCanvas: React.FC = () => {
  ...
  const { selectedColor, drawingTool, panOffset, zoom, setPanOffset, setZoom, shapeText } = useUIStore();
  ...
  const getShapeBounds = (shape: DrawingStroke) => {
    // ... (logic to get bounding box of a shape)
    return { minX: 0, minY: 0, maxX: 0, maxY: 0 };
  };

  const drawShape = useCallback((ctx: CanvasRenderingContext2D, shape: DrawingStroke) => {
    ...
    switch (shape.type) {
      case 'text':
        const fontSize = 24; // You might want to make this dynamic
        ctx.font = `${fontSize / zoom}px 'Inter', 'Segoe UI Emoji', 'Apple Color Emoji', 'Noto Color Emoji'`;
        ctx.textAlign = 'left';
      ...
      case 'rectangle':
        ...
        if (shape.text) {
            const fontSize = Math.min(Math.abs(shape.width), Math.abs(shape.height)) * 0.5;
            ctx.font = `${fontSize / zoom}px 'Inter', 'Segoe UI Emoji', 'Apple Color Emoji', 'Noto Color Emoji'`;
  ...
  const getStrokeUnderPoint = useCallback((point: {x: number, y: number}): string | null => {
    ...
    for (let i = existingStrokes.current.length - 1; i >= 0; i--) {
        ...
        switch (shape.type) {
            case 'stroke':
                if (shape.points.some(p => Math.sqrt(Math.pow(p.x - point.x, 2) + Math.pow(p.y - point.y, 2)) < eraserRadius)) return shape.id;
                break;
            default:
                const { minX, maxX, minY, maxY } = getShapeBounds(shape);
                if (point.x > minX && point.x < maxX && point.y > minY && point.y < maxY) return shape.id;
                break;
        }
    }
    return null;
  ...
  useEffect(() => {
    const loop = () => {
      ...
      if (drawingTool === 'eraser') {
        bufferCtx.beginPath();
        bufferCtx.arc(lastMousePosition.current.x, lastMousePosition.current.y, 10 / zoom, 0, Math.PI * 2);
  ...
  useEffect(() => {
    ...
    const handleYjsUpdate = (transaction: Y.Transaction) => {
        const yStrokeEvent = transaction.events.find(e => e instanceof Y.YArrayEvent && e.path.length === 1 && e.path[0] === 'drawingStrokes');

        if (yStrokeEvent) {
            console.log("Remote change detected, reconciling...");
            existingStrokes.current = yStrokes.toArray();
            db.drawingStrokes.clear().then(() => db.drawingStrokes.bulkPut(existingStrokes.current));
            redrawAllShapes();
        }
      };
    ydoc.on('afterTransaction', handleYjsUpdate);
    return () => ydoc.off('afterTransaction', handleYjsUpdate);
  }, [isLoaded, redrawAllShapes]);

  useEffect(() => {
    if (!isLoaded) return;
    const reconcileAndLoadStrokes = async () => {
      const localStrokes = await db.drawingStrokes.where('taskId').equals(activeTaskId || '').toArray();
  ...
  const getScreenToWorldCoordinates = (screenX: number, screenY: number) => {
  ...
  const eraseAtPoint = useCallback((point: {x: number, y: number}) => {
    ...
    if (strokeIdToDelete) {
        const indexToDelete = yStrokes.toArray().findIndex(s => s.id === strokeIdToDelete);
        if (indexToDelete > -1) {
  ...
  }, [isLoaded, drawingTool, selectedColor, panOffset, zoom, eraseAtPoint, textInput]);
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    switch (drawingTool) {
        case 'pen': updatedStroke = { ...localStroke.current, points: [...(localStroke.current.points || []), { ...currentPoint, pressure: e.pressure || 0.5 }] }; break;
        case 'rectangle': updatedStroke = { ...localStroke.current, x: startPoint.x, y: startPoint.y, width: currentPoint.x - startPoint.x, height: currentPoint.y - startPoint.y }; break;
        case 'circle': const dx = currentPoint.x - startPoint.x; const dy = currentPoint.y - startPoint.y; updatedStroke = { ...localStroke.current, cx: startPoint.x, cy: startPoint.y, radius: Math.sqrt(dx*dx + dy*dy) }; break;
        case 'triangle': const p1 = startPoint; const p2 = { x: currentPoint.x, y: currentPoint.y }; const midX = (p1.x + p2.x) / 2; const p3 = { x: midX, y: p1.y - (p2.x - p1.x) * 0.5 }; updatedStroke = { ...localStroke.current, type: 'triangle', p1, p2, p3 }; break;
    }
    if (updatedStroke) {
        localStroke.current = updatedStroke;
        awareness?.setLocalStateField('drawing', updatedStroke);
    }
  }, [isDrawing, startPoint, drawingTool, panOffset, zoom, setPanOffset, eraseAtPoint]);
  ...
  const handlePointerUp = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (drawingTool !== 'pan' && drawingTool !== 'eraser' && localStroke.current.type) {
        ...
        if(finalShape.type === 'pen' && (!finalShape.points || finalShape.points.length < 2)) return;
    ...
    localStroke.current = {};
    setStartPoint(null);
  }, [isDrawing, drawingTool, startPoint, shapeText, redrawAllShapes]);
  ...
  return (
    <CanvasContainer onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerLeave={handlePointerUp} onWheel={handleWheel} $tool={drawingTool}>
